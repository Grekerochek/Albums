...
import com.alexander.documents.api.PhotoRequestDelete
...
class AlbumDetailsActivity : AppCompatActivity() {
    ...
    private val photosAdapter: PhotosAdapter by lazy(LazyThreadSafetyMode.NONE) {
        PhotosAdapter(::onPhotoClick, ::onPhotoLongClick,::onPhotoRemoveClick )
    }

    private var stateIsEdit = false
    ...
    private fun initViews() {
        ...
        toolbarButtonAdd.visibility = if (album.id < 0) View.GONE else View.VISIBLE
        ...
        recyclerViewPhotos.itemAnimator = CommonRecyclerViewAnimator()
    ...
    private fun onPhotoLongClick(): Boolean {
        if (stateIsEdit) {
            return true
        }
        stateIsEdit = true
        toolbarButtonBack.setOnClickListener { resetState() }
        photosAdapter.animateState()
        return true
    }

    private fun onPhotoRemoveClick(photo: Photo) {
        containerView.isRefreshing = true
        VK.execute(PhotoRequestDelete(photo.id), object : VKApiCallback<Int> {
